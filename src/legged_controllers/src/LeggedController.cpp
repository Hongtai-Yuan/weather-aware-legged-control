...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh) {
  ...
  for (const auto& name : leggedInterface_->modelSettings().contactNames3DoF) {
    //std::cerr<<name<<std::endl;//LF_FOOT RF_FOOT LH_FOOT RH_FOOT
  ...
  switch_controller_client_ =
    controller_nh.serviceClient<controller_manager_msgs::SwitchController>(
        "/controller_manager/switch_controller");
  subStart_ = controller_nh.subscribe<std_msgs::Float32>(
    "/start_controller", 1,
    &LeggedController::StartControllerCallback, this);
  contactForce_pub_ = controller_nh.advertise<std_msgs::Float64MultiArray>("contacti_flag", 10);
  subEmgstop_ = controller_nh.subscribe<std_msgs::Float32>("/emergency_stop", 1,
...
void LeggedController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  for (size_t j = 0; j < leggedInterface_->getCentroidalModelInfo().actuatedDofNum; ++j) {
     ...
     if (emergencyStopFlag_)
      {
        std::cerr<<"emergencyStopFlag"<<std::endl;
        hybridJointHandles_[j].setCommand(0, 0, 0, 3, 0);
      }
      else
      {//0，3
        hybridJointHandles_[j].setCommand(posDes(j), velDes(j), 4.0, 5, torque(j));
      }
...
void LeggedController::updateStateEstimation(const ros::Time& time, const ros::Duration& period) {
  ...
  // std::cerr<<"mode: "<<stanceLeg2ModeNumber(cmdContactFlag)<<std::endl;
  ...
  force_msg.data.resize(4);
  
  for(int i=0;i<4;i++)
  {
    force_msg.data[i] = contactFlag[i];
  }

  contactForce_pub_.publish(force_msg);
}
...
void LeggedController::EmergencyStopCallback(const std_msgs::Float32::ConstPtr& msg)
{
  ...
  ROS_INFO("enter emergencyStop");
}

void LeggedController::StartControllerCallback(const std_msgs::Float32::ConstPtr& msg)
{
  controller_manager_msgs::SwitchController srv;

        srv.request.start_controllers.push_back("controllers/legged_controller");
        // 不停任何 controller 可以不填
        // srv.request.stop_controllers.clear();

       srv.request.strictness =
    controller_manager_msgs::SwitchControllerRequest::BEST_EFFORT;
        // 或 STRICT
        // srv.request.strictness = controller_manager_msgs::SwitchController::STRICT;
        ...
        if (switch_controller_client_.call(srv))
        {
            if (srv.response.ok)
            {
                ROS_INFO("Legged controller started successfully.");
            }
            else
            {
                ROS_WARN("SwitchController service returned false.");
            }
        }
        else
        {
